...
# EDIT: will remove this method in the next stage (making tables and figures with results) so that when readers review the "res2" file on github (which contains the "cleaned" simulation results), they can see for themselves that this method gives extremely high coverage rates, which is clearly inconsistent with Yang et al. 2016
```
...
```{r}
#save as a csv file
write.csv(res2, row.names = F, file = paste0("data/cleaned/cleaned SS_all models_2025-09-08_1000_sims.csv", sep = ""))
```
