---
title: "simulation study 2025_peer review revised"
...
Helps find computation time
```{r}
find_time <- function(x) {
  time <- x$toc - x$tic # elapsed time is the stop time minus the start time
  return(time)
}
```
...
## testing that discrete models work
```{r}
n <- 10000000
x2 <- rbinom(n, 1, prob_x2) 
x1 <- make_cat_x1(n = n, x2 = x2)

table(x1)/n # proves that there is an equal probability of having x1 = 0, 1, 2, or 3.
table(x2)/n # proves that 20% are coded as '1', or low SES.
```



Work computer:
First iteration takes 2 min 34 seconds when sample sizes of n = 100, 250, 500, 1000 used
First iteration takes 2 min 7 seconds when sample sizes of n = 100, 250, 500, 750 used

Personal computer:
First iteration takes 1 min 14 seconds when sample sizes of n = 100, 250, 500, 750 used
First iteration takes 1 min 47 seconds when sample sizes of n = 100, 250, 500, 750 used with categorical models added
First iteration takes 2 min 18 seconds when sample sizes of n = 100, 250, 500, 750 used with all 9 models

# make table
```{r}
table <- setNames(data.frame(matrix(data = NA, nrow = m*length(taus)*length(paras)*nsims, 
                                    ncol = 10)), 
                  c("method", "beta", "lb", "ub", "true_beta", "tau", "model", "n", "time", "converge_attempts")) #set column names
```
  ...
  for(j in 1:length(paras)){
    ...
    for(k in 1:length(taus)){
      ...
      if (model %in% heavy_tailed_models){
        ...
        true_beta <- beta1 + qnorm(tau, mu_u, sigma_u)*(alpha_x1) # errors are normally distributed 
      } 
      ...
      time_rq_nodith <- toc(log = TRUE, quiet = TRUE) # stop timer, record computation time

      ### rank with iid error intervals
      tic()
      riid_nodith_coef <- (summary(rq_model_nodith, se = "rank", iid = T, alpha = z_lower * 2)$coefficients)
      time_riid_nodith <- toc(log = TRUE, quiet = TRUE)

      ### xy bootstrap intervals
      
      if(model %in% cnts_models){ # Undithered xy method crashes R when x1 is continuous. So I will save a matrix of NAs instead of running crash-prone code
        xy_nodith_coef <- matrix(data = NA, nrow = 2, ncol = 2)
        
        # Save time elapsed as NA, as this model was not run
        time_xy_nodith$tic <- NA
        time_xy_nodith$toc <- NA
        ...
        time_xy_nodith <- toc(log = TRUE, quiet = TRUE)
      ...
      repeat{ # Remake model again if model fails to converge
        ...
        if(model_converged == T){ # if the model converged
          # stop the timer
          time_bayesQR <- toc(log = TRUE, quiet = TRUE)
          ...
          bayes_unadj <- c(bayesQR_coef[1], # beta
                           bayesQR_coef[2], # lb
                           bayesQR_coef[3]) # ub
          time_bayesQR_unadj <- toc(log = TRUE, quiet = TRUE)
          ...
          time_bayesQR_adjv1 <- toc(log = TRUE, quiet = TRUE)
          ...
          bayes_adjv2 <- c(bayes_adj_sw[1], # beta
                           bayes_adj_sw[2], # lb
                           bayes_adj_sw[3]) # ub
          time_bayesQR_adjv2 <- toc(log = TRUE, quiet = TRUE)
        ...
        if(repeats_bqr == 20){ # if model fails to converge too many times
          ...
          # Because we failed to converge too many times and got no results,
          # we cannot calculate the time for this iteration. So we will store it as NA,
          # and make note that iterations that failed to converge were not included in 
          # computation time. Otherwise the computation time would go to infinity (as if it does not
          # converge after a couple of attempts, it seems to never converge even after letting it 
          # run for hours on end)
          time_bayesQR$tic <- NA
          time_bayesQR$toc <- NA
          time_bayesQR_unadj$tic <- NA
          time_bayesQR_adjv1$tic <- NA
          time_bayesQR_adjv2$tic <- NA
          time_bayesQR_unadj$toc <- NA
          time_bayesQR_adjv1$toc <- NA
          time_bayesQR_adjv2$toc <- NA
      ...
      # Calculate computation time: time to make frequentist model + time to make rank intervals
      table[skip +1, 9] <- find_time(time_rq_nodith) + find_time(time_riid_nodith)
      ...
      table[skip +2, 9] <- find_time(time_rq_nodith) + find_time(time_xy_nodith)
      ...
      table[skip +3, 9] <- find_time(time_rq_dith) + find_time(time_riid_dith)
      ...
      table[skip +4, 9] <- find_time(time_rq_dith) + find_time(time_xy_dith)
      ...
      table[skip +5, 9] <- find_time(time_bayesQR) + find_time(time_bayesQR_unadj)
      table[skip +5, 10] <- repeats_bqr
      ...
      table[skip +6, 9] <- find_time(time_bayesQR) + find_time(time_bayesQR_adjv1)
      table[skip +6, 10] <- repeats_bqr
      ...
      table[skip +7, 9] <- find_time(time_bayesQR) + find_time(time_bayesQR_adjv2)
      table[skip +7, 10] <- repeats_bqr
...
#save as a csv file
write.csv(table, row.names = F, 
          file = paste0("data/raw/QR sim study_all models", "_",  Sys.Date(), "_", nsims, "_sims.csv", sep = ""))
```
