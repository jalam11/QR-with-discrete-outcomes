---
title: "performance figs_peer review updated"
author: "Josh Alampi"
date: "2025-08-26"
...
res <- read.csv("data/cleaned/cleaned SS_all models_2025-09-08_1000_sims.csv") %>% 
  filter(method != "bayes_adjv1") %>% 
...
int_method_labels <- c("Rank (not dith.)", "Rank (dith.)", 
                      ...
                      "Bayes. (unadj.)", "Bayes. (adj.)")

int_method_labels_cov <- c("Rank (not\ndithered)", "Rank\n(dithered)", 
                          "xy-bootstrap\n(not dithered)", "xy-bootstrap\n(dithered)",
                          "Bayesian\n(unadjusted)", "Bayesian\n(adjusted)")
...
res <- res %>% 
  ...
  mutate(model_simple = model) %>% # make new model variable keeping the short names
  mutate(model = case_when(model == 1 ~ model_labels[1],
                           ...
                           model == 6 ~ model_labels[6],
                           model == 7 ~ model_labels[7])) 
...
y_empSE_rel <- "Relative (%) difference in the variance of point estimates (±1 Monte Carlo SE)"
y_blank <- ""
...
## colours
Colour blind and printer freindly!
...
## bias
```{r}
make_bias_plot <- function(res, title = NULL, ystrip_angle = 270) {
  fig_bias <- res %>%
    ...
    gg_geom_point(x = tau, y = (bias), 
                  ...
                  colour = beta_method, col_title = col_title, colour_vec = col_3,
                  legend.direction = "horizontal",
                  
                  title = title, ylab = y_bias, xlab = x_lab,
                  coord_flip = F, ystrip_angle = ystrip_angle
                  # panel.spacing.x = 0.2, panel.spacing.y = 0.05
                  ) + 
    # Improve readability by changing the vertical grid lines
    theme(panel.grid.major.x = element_blank()) + # get rid of vertical grid line at 0.1, 0.5, 0.9
    geom_vline(xintercept = c(1.5, 2.5), colour = "grey", linewidth = 0.5) + # add grid lines between the taus
...
## empSE_rel
```{r}
make_empSE_rel_plot <- function(res, title = NULL, ystrip_angle = 270) {
  plot <- res %>% 
    ...
    gg_geom_point(x = tau, y = empSE_rel, 
                  ...
                  shape = blank, shape_vec = c(1), shape_title = NULL,
                  colour = beta_method_ref, col_title = col_title, colour_vec = col_3,
                  p_dodge = T, 
                  facet_type = "grid", hori = "model",  vert = "n",
                  legend.direction = "horizontal",
                  scales = "free_x",
                  title = title, ylab = y_empSE_rel, xlab = x_lab,
                  yint = 0, ystrip_angle = ystrip_angle,
                  # panel.spacing.x = 0.2, panel.spacing.y = 0.05
                  )  +
    # Improve readability by changing the vertical grid lines
    theme(panel.grid.major.x = element_blank()) + # get rid of vertical grid line at 0.1, 0.5, 0.9
...
## cov
```{r}
make_cov_plot <- function(res, title = NULL, cov_limit = 0.65, ystrip_angle = 270) {
  cov_limit <- cov_limit
  ...
  plot <- gg_geom_point(.data = data_tmp, x =tau, y = cov, ymin = cov - (cov_se), ymax = cov + (cov_se),
                        shape = blank, shape_vec = c(1), shape_title = NULL,
                        colour = beta_method_int, col_title = col_title, colour_vec = col_3,
                        legend.direction = "horizontal",
                        facet_type = "grid", hori = "model", vert = "int_method_cov",  p_dodge = F,
                        ylab = y_cov, xlab = x_lab, title = title,
                        text_size_minor = 8, text_size_major = 9.5, yint = 0.9, 
                        limits = c(cov_limit,1), coord_flip = F, ystrip_angle = ystrip_angle,
                        panel.spacing.x = 0.05, panel.spacing.y = 0.05
                        ) + 
...
fig_cov_750 <- res %>% 
  filter(n == "n = 750") %>% 
  make_cov_plot(., title_c)

ggsave(file = paste0("results/fig_cov_750", "_",  Sys.Date(), ".png"),
       width = fig_width, height = fig_height, dpi = 600)
...
```{r}
make_intwidthEmpSE_plot <- function(res, title = NULL, ylab = "", ystrip_angle = 270,
  ...
  plot <- res %>% 
    ...
    gg_geom_point(x = int_method, y = intwidth_empSE, 
                  ...
                  text_size_minor = 7, text_size_major = 9.5,
                  yint = NULL, coord_flip = F, ystrip_angle = ystrip_angle,
                  panel.spacing.x = 0.025, panel.spacing.y = 0.05) + 
    scale_y_continuous(labels = number_format(accuracy = 0.1) ) +
