---
title: "Histograms of betas_updated"
author: "Josh Alampi"
date: "2025-08-26"
...
make_hist_plot <- function(data, title = "", breaks = waiver(), xlims = c(),
                           ...
                           panel.spacing.x = 0.015, panel.spacing.y = 0.05,
                           x_angle = 0, ystrip_angle = 0) {
  if(make_smaller == T){
    text_size_minor <- 6.5
    text_size_major <- 7.5
    text_size_title <- 8.5
    panel.spacing.x = 0.0075
    panel.spacing.y = 0.005
  }
...
histograms_tau50 <- data %>% 
  ...
  make_hist_plot(., ystrip_angle = 270)
...
title_j <- "J) n = 750, τ = 0.1"
title_k <- "K) n = 750, τ = 0.5"
title_l <- "L) n = 750, τ = 0.9"

data_100 <- filter(data, n == "n = 100")
data_250 <- filter(data, n == "n = 250")
data_500 <- filter(data, n == "n = 500")
data_750 <- filter(data, n == "n = 750")

hist_abc <- ggarrange(data_100 %>% 
                        filter(tau == 0.1) %>% 
                        make_hist_plot(., title_a, make_smaller = T, ystrip_angle = 270),
                      data_100 %>% 
                        filter(tau == 0.5) %>% 
                        make_hist_plot(., title_b, make_smaller = T, ystrip_angle = 270),
                      data_100 %>% 
                        filter(tau == 0.9) %>% 
                        make_hist_plot(., title_c, make_smaller = T, ystrip_angle = 270),
                      nrow = 3)  
...
hist_def <- ggarrange(data_250 %>% 
                        filter(tau == 0.1) %>% 
                        make_hist_plot(., title_d, make_smaller = T, ystrip_angle = 270),
                      data_250 %>% 
                        filter(tau == 0.5) %>% 
                        make_hist_plot(., title_e, make_smaller = T, ystrip_angle = 270),
                      data_250 %>% 
                        filter(tau == 0.9) %>% 
                        make_hist_plot(., title_f, make_smaller = T, ystrip_angle = 270),
                      nrow = 3)  
...
hist_ghi <- ggarrange(data_500 %>% 
                        filter(tau == 0.1) %>% 
                        make_hist_plot(., title_g, make_smaller = T, ystrip_angle = 270),
                      data_500 %>% 
                        filter(tau == 0.5) %>% 
                        make_hist_plot(., title_h, make_smaller = T, ystrip_angle = 270),
                      data_500 %>% 
                        filter(tau == 0.9) %>% 
                        make_hist_plot(., title_i, make_smaller = T, ystrip_angle = 270),
                      nrow = 3)  
ggsave(file = paste0("results/hist_n500", "_",  Sys.Date(), ".png"), 
       width = fig_width, height = fig_height_long, dpi = 600)

hist_jkl <- ggarrange(data_750 %>% 
                        filter(tau == 0.1) %>% 
                        make_hist_plot(., title_j, make_smaller = T, ystrip_angle = 270),
                      data_750 %>% 
                        filter(tau == 0.5) %>% 
                        make_hist_plot(., title_k, make_smaller = T, ystrip_angle = 270),
                      data_750 %>% 
                        filter(tau == 0.9) %>% 
                        make_hist_plot(., title_l, make_smaller = T, ystrip_angle = 270),
                      nrow = 3)  
ggsave(file = paste0("results/hist_n750", "_",  Sys.Date(), ".png"), 
       width = fig_width, height = fig_height_long, dpi = 600)
...
# cut content
don't need this, I updated the make_hist_plot() function with the make_smaller arguement instead
```{r}
